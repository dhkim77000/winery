...
def to_kg():
    ...
    df = pd.read_csv('/opt/ml/wine/data/item_data_expand.csv', encoding = 'utf-8-sig')
    wine = pd.read_csv('/opt/ml/wine/data/wine_df.csv', encoding = 'utf-8-sig')
...
def main(args):
    kg = pd.read_csv('/opt/ml/wine/wine_kg.csv')
    kg = kg.dropna()
    for col in kg.columns: kg[col] = kg[col].astype(str)
...
if __name__ == "__main__":
    ...
    np.save('/opt/ml/wine/EDA/graph/embedding/entity_embedding.npy', entity_embedding)
    relation_to_id = result.training.relation_to_id
    entity_to_id = result.training.entity_to_id
    with open('/opt/ml/wine/EDA/graph/embedding/relation2idx.json','w') as f: 
        json.dump(relation_to_id,f)
    with open('/opt/ml/wine/EDA/graph/embedding/entity2idx.json','w') as f: 
