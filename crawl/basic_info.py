    ...
    for url in tqdm(urls):
        if url not in done:
            ...
            if len(datas) == 100:
                ...
                df.to_csv('/opt/ml/wine/data/basic_info.csv', encoding = 'utf-8-sig',index= False)
                datas.clear()
                with open('/opt/ml/wine/data/basic_done.pkl','wb') as f: pickle.dump(done,f)
    print('Saving')
    df = write_data(df, datas)
    df.to_csv('/opt/ml/wine/data/basic_info.csv', encoding = 'utf-8-sig',index= False)
    datas.clear()
...
if __name__ == '__main__':
    ...
    try:
        with open('/opt/ml/wine/data/basic_done.pkl', 'rb') as f: done  = pickle.load(f)
    except:
        done = set()
    ...
    #df = pd.DataFrame()
    try:
        df = pd.read_csv('/opt/ml/wine/data/basic_info.csv', encoding = 'utf-8-sig')
