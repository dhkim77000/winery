...
export default function Mbti({navigation: {navigate}, route}) {
  ...
  const isBack = () => { // back 눌렀을 때 전 문제로 돌아가기
    setQuestionNum(questionNum - 1);
    result.pop()
    setResult([...result]);
  }
    ...
    return (
      <View style={styles.wrapper}>
        <View style={styles.progressBarOut}>
          <View style={barStyle(`${(questionNum+1)*10}%`).progressBarIn}>
          </View>
        </View>
