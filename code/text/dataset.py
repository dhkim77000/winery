...
import re
# OPTIONAL: if you want to have more information on what's happening, activate the self.logger as follows
import logging
    ...
    def __init__(
        ...
        with FileLock(lock_path):
            if os.path.exists(cached_features_file) and not overwrite_cache:
                ...
                self.logger.info(f"Creating features from dataset file at {directory}")
                ...
                self.logger.info(f"Creating examples from {len(self.documents)} documents.")
                self.examples = []
