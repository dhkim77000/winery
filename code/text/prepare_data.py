...
def main(args):
    nltk.data.path.append('/home/dhkim/server_front/winery_AI/winery/code/nltk_data')
    
    # Now download the punkt tokenizer data
    nltk.download('punkt')
    print('-------------------Reading Datas-------------------')
    with open('/home/dhkim/server_front/winery_AI/winery/code/data/feature_map/item2idx.json','r') as f: item2idx = json.load(f)
    with open('/home/dhkim/server_front/winery_AI/winery/code/data/feature_map/price_vocab.json','r') as f: price_vocab = json.load(f)
    with open('/home/dhkim/server_front/winery_AI/winery/code/data/feature_map/note.json','r') as f: notes_data = json.load(f)

    basic_info = pd.read_csv('/home/dhkim/server_front/winery_AI/winery/data/basic_info_total.csv')
    wine_df = pd.read_csv('/home/dhkim/server_front/winery_AI/winery/data/wine_df.csv')
    #########################WINE LABEL#########################  
...
#########################REVIEW DATA#########################
    try:
        ...
        if args.run == 0:
            ...
            review_df.to_csv('/home/dhkim/server_front/winery_AI/winery/data/review_df_cleaned.csv',index = False)

    except Exception as e:
        print(e)
        print('-------------------Processing review text-------------------')
        review_df = pd.read_csv('/home/dhkim/server_front/winery_AI/winery/data/review_df_total.csv',encoding = 'utf-8-sig').loc[:,['user_url','rating','text','wine_url']]
        tqdm.pandas() 
...
#########################NOTE LABEL#########################
    ...
    if args.label == 1:
        ...
        gc.collect()
        with open('/home/dhkim/server_front/winery_AI/winery/code/data/feature_map/grape2idx.json','w') as f: json.dump(grape2idx, f)
        with open('/home/dhkim/server_front/winery_AI/winery/code/data/feature_map/country2idx.json','w') as f: json.dump(country2idx, f)
        with open('/home/dhkim/server_front/winery_AI/winery/code/data/feature_map/winetype2idx.json','w') as f: json.dump(winetype2idx, f)
...
#######Train#############################################################################
    ...
    parser.add_argument("--save_path", default="/home/dhkim/server_front/winery_AI/winery/data/", type=str)
